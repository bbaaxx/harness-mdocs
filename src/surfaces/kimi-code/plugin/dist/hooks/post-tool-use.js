...
// src/surfaces/kimi-code/cli/post-tool-use.ts
var post_tool_use_exports = {};
__export(post_tool_use_exports, {
  runPostToolUse: () => runPostToolUse
});
...
async function runPostToolUse() {
  ...
  core.managers.audit.append({
    timestamp: (/* @__PURE__ */ new Date()).toISOString(),
    type: "tool",
    initiativeId: activeInitiativeId || void 0,
    step,
    details: { toolName, args: toolArgs }
  });
}
if (require.main === module) {
  runPostToolUse().then(() => process.exit(0)).catch(() => process.exit(0));
}
