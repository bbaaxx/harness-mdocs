...
description: Use when creating, updating, or querying mdocs initiatives in Claude Code sessions.
---

# Mdocs Initiative Management For Claude Code

Initiatives live in `./mdocs/initiatives/`. Filename format: `<id-slug>--<YYYY-MM-DD>.md`.

Prefer the MCP `mdocs` aggregate tool (`command` + `args`). Fall back to the `mdocs` CLI via Bash. Last resort: edit files directly, then run `mdocs_validate`.
