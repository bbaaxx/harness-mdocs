...
const utils_1 = require("../../core/commands/utils");
 ...
 */
function createClaudeCodeHooks(core) {
    return {
        /**
        ...
        preToolUse(payload) {
            ...
            return {
                allowed: false,
                step,
                reason: `mdocs workflow gate: "${payload.tool_name}" is blocked at step ${step}. ` +
         ...
         */
        postToolUse(payload) {
            const { toolName } = (0, translate_1.toCore)(payload);
            const step = core.managers.workflow.getCurrentStep();
            ...
            core.managers.audit.append({
                ...
                step,
                details: { toolName, args: (0, translate_1.toCore)(payload).toolArgs }
            });
            ...
            if (step !== 'IDLE' && activeInitiativeId) {
                (0, lock_1.withLock)(core.mdocsRoot, 'initiative-progress', () => {
                    const fileName = (0, utils_1.findInitiativeFilename)(core.mdocsRoot, core.managers.initiatives, activeInitiativeId);
                    if (!fileName)
                        return;
