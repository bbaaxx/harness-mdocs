...
export function translateToolName(claudeToolName: string): string {
  if (claudeToolName in TOOL_NAME_MAP) return TOOL_NAME_MAP[claudeToolName];
 ...
 */
export function parseHookStdin(raw: string): ClaudeHookPayload | null {
  try {
    const parsed = JSON.parse(raw);
    if (parsed && typeof parsed === 'object') return parsed as ClaudeHookPayload;
    return null;
  } catch {
    return null;
  }
...
export function toCore(payload: ClaudeHookPayload): { toolName: string; toolArgs: Record<string, unknown> } {
  return {
