...
function isCategoryWikiRef(ref: string, wiki: { category: string; id: string }): boolean {
...
export class MdocsLinter {
  ...
  private lintGraph(): LintResult[] {
    ...
    const initiativeData: { id: string; slug: string; aliases: string[]; status: string; relatedWiki: string[]; filePath: string }[] = [];
    for (const filePath of this.listInitiativeFiles()) {
      try {
        const content = fs.readFileSync(filePath, 'utf8');
    ...
    const wikiData: { id: string; category: string; relatedInitiatives: string[]; sourceInitiatives: string[]; relatedWiki: string[]; lifecycle?: string; filePath: string }[] = [];
    if (fs.existsSync(wikiDir)) {
      for (const filePath of this.rootWikiFiles()) {
        try {
          const content = fs.readFileSync(filePath, 'utf8');
    ...
    const initiativeAliases = new Map<string, string>();
    ...
    const resolveWikiRef = (ref: string) => wikiData.find(wiki => wiki.category
    ...
    for (const wiki of wikiData) {
      ...
      const wikiRef = wiki.category ? `${wiki.category}/${wiki.id}` : wiki.id;
      const hasSelfRelatedInitiative = wiki.relatedInitiatives.some(initRef =>
        (canonicalInitiatives.has(initRef) ? initRef : initiativeAliases.get(initRef) ?? initRef) === wiki.id
      );
  ...
  private suggest(value: string, choices: string[]): string | null {
