...
## Plan
- [ ] In `src/core/validation/linter.ts` `lintInitiative` (lines ~232-306): when the linter's contract is `metadata-only`, gate off the body-section and required-field deductions; keep lifecycle telemetry (lines ~308-347) and switch the expected-duration read at line ~314 to `readExpectedDurationRaw`.
- [ ] In `lintWiki` (lines ~368-423): derive expected category from the parent dir and accept singular/plural equality; make `created` optional with `updated` fallback; demote the missing-`tags` consequence to non-fatal.
- [ ] Update `tests/core/linter.test.ts` and `tests/core/lifecycle-lint.test.ts` for the new tolerant outcomes; add cases for a metadata-only initiative scoring pass and a singular-category wiki page not warning.

## Acceptance Criteria
- A thin metadata-only `_status.md` lints `passed:true`; a wiki page with `category: system` in dir `systems` and no `created` raises no category/created error.
- Lifecycle warnings (long-running-active etc.) fire correctly off hyphenated `expected-duration`.
- `full` mode lint outcomes unchanged.
